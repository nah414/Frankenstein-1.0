#!/usr/bin/env python3
"""
FRANKENSTEIN 1.0 - AQT Alpine Adapter
Trapped-ion quantum computing platform.
Requires: pip install aqt-client
"""
...
class AqtAdapter(ProviderAdapter):
    def __init__(self, credentials: Optional[Dict] = None):
        super().__init__("aqt", ProviderCategory.QUANTUM_HARDWARE, credentials)
        self._client = None
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.error_message = "aqt-client not installed. Run: pip install aqt-client"
            return False
        ...
        if not token:
            ...
            self.error_message = (
                "Missing 'token'. Get your AQT API key at https://www.aqt.eu → Account → API key. "
                "Then run: credentials save aqt --token YOUR_TOKEN"
            )
            return False
